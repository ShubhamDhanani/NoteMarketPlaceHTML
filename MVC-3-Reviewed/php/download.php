...
<?php
        ...
        $paid=$result['IsPaid'];
        $allowdownload=$result['IsSellerHasAllowedDownload'];
        if($paid==0){
            $atdownloaddate = "CURRENT_TIMESTAMP()";
        }
        elseif($paid==1 && $allowdownload==1){
            $atdownloaddate = "CURRENT_TIMESTAMP()";
        }
        else{
            $atdownloaddate = "NULL";
...
if($update or $insert){
    $sele= "select * from downloads Where NoteID=$noteid and Downloader=$user";
    $seleq = mysqli_query($con,$sele);
    $resu = mysqli_fetch_assoc($seleq);
    $downallow = $resu['IsSellerHasAllowedDownload'];
    $ispaidd =$resu['IsPaid'];
    ...
    if($ispaidd==1 and $downallow==0){
        $_SESSION['show']=1;
        header("location:../front/note-details.php?id=$noteid");
    }
    if($ispaidd==1 and $downallow==1){        
        $_SESSION['download']=1;
        header("location:../front/note-details.php?id=$noteid");
    }
}
