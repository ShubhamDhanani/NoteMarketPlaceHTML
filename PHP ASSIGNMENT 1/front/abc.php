        ...
        if($is_note_inserted == "true") {
            ...
            if(!empty($last_inserted_id)) {
                ...
                for($i=0; $i<$atta_count; $i++) {
                    ...
                    $note_preview_filecheck = strtolower(end($note_date_fileext));
                    $note_data_ext = end($note_date_fileext);
                    
                    $atta_path = "../Members/$user_id/$last_inserted_id/Attachements/$notes_data_filename"; 
                    
                    $insert_attachements = "INSERT INTO `sellernotesattachements`(`NoteID`, `FileName`, `FilePath`, `CreatedDate`, `CreatedBy`, `ModifiedDate`, `ModifiedBy`, `IsActive`) VALUES ($last_inserted_id, '$notes_data_filename', '$atta_path', current_timestamp(), '1', current_timestamp(), '1', b'1')";
                    
                    $last_attachement_id = mysqli_insert_id($conn);
                  
                    
                    date_default_timezone_set('Asia/Kolkata');
                    
                    $store_name_atta = $last_attachement_id . "_" . date("dmyhis") . "." . $note_data_ext;
            ...
            <?php 
                ...
                if($uquery) {
                    ...
                    if(!$mail->send()) {
                        ...
                        echo "window.location.href='http://localhost/srspractice/SRS_Notes/front/dashboard.php';";
                    }    
                }else {
...
//    $insert_note_query2 = "INSERT INTO `sellernotes` (`SellerID`, `Status`, `ActionedBy`, `AdminRemarks`, `PublishedDate`, `Title`, `Category`, `DisplayPicture`, `NoteType`, `NumberofPages`, `Description`, `UniversityName`, `Country`, `Course`, `CourseCode`, `Professor`, `IsPaid`, `SellingPrice`, `NotesPreview`, `CreatedDate`, `CreatedBy`, `ModifiedDate`, `ModifiedBy`, `IsActive`) VALUES ('46', '6', '46', 'best', current_timestamp(), '$title', $category, '$display_picture_filename', $note_type, $number_of_pages, '$description', '$institute_name', $country, '$course_name', '$course_code', '$professor_name', b'$sellingtype', $sellprice, '$note_preview_filename_db', current_timestamp(), '1', current_timestamp(), '1', b'1')";
    
    
//    
//                while($row = mysqli_fetch_assoc($last_query)) {
//                    $seller_id = $row['SellerID'];
//                    
//                    $query = "UPDATE sellernotes SET status = 7 WHERE SelerID = $seller_id";
//                    $uquery = mysqli_query($conn, $query);
//                    if($uquery) {
//                       echo 'window.location.href("http://localhost/srspractice/SRS_Notes/front/dashboard.php");';
//                    }
