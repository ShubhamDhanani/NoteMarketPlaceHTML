...
if(isset($_POST['action'])){
    $sql = 'SELECT sellernotes.* ,AVG(Ratings) AS avgratings FROM sellernotes LEFT JOIN sellernotesreviews ON sellernotesreviews.NoteID=sellernotes.ID  GROUP BY sellernotes.ID ';
    ...
    if(isset($_POST['type'])){
        ...
        $type = $_POST['type'];
        if($type==""){$sql .='HAVING NoteType!=""';}
        else{$sql .="HAVING NoteType IN($type)";}
                ...
                <div class="card-img-top">
                    <img src="../Member/<?php echo $result['SellerID'].'/'.$result['ID'].'/'.$result['DisplayPicture'];?>" alt="note image" class="note-img img-fluid">
                        ...
                        <div class="details card-body">
                            <a href="note-details.php?id=<?php echo $result['ID'];?>" style="text-decoration: none;"><h4 class="card-title" style="font-size: 30px;line-height: 35px;"><?php echo $result['Title'];?></h4></a>
                    ...
                    <div class="ratings">
                        <div class="rate" style="margin-top:2px;">
                          <span class="search_rate">
                           <?php
                                ...
                                while($i<6){
                                    if($i<=$rate){
                                        ?>
                                        <i class="fas" style="font-size:25px;line-height:30px;font-style:normal;color:#FDCC0D">★</i>
                                        <?php
                                    }
                                    else{
                                        ?>
                                        <i class="far" style="font-size:25px;line-height:30px;font-style:normal;color:gray">☆</i>
                                        <?php
                                    }
                                    $i++;
                                }
                        ...
                        <p><?php echo $totalrate; ?> reviews</p>
                    </div>
                </div>
            </div>
        </div>
